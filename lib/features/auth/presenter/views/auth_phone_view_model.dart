...
import 'package:authphone_firebase/app/auto_injector.dart';
...
class AuthPhoneViewModel extends BaseViewModel
  ...
  sign(int phoneNumber) async {
    phoneNumberEntity = PhoneNumberEntity(phoneNumber: phoneNumber);

    catchLog(message: 'run sign method in viewmodel 🪱', name: "run-sign");
    isLoading = true;
    notifyListeners();
    await signWithPhoneNumberUsecase(phoneNumberEntity!).then((value) {
      isLoading = false;
      notifyListeners();
      statusCheck(
          status: returnData(data: value).status,
